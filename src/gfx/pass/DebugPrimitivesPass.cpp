...
void DebugPrimitivesClearPass::Render(RenderGraph& graph, VKW::Context& context)
{
    ...
    dependencyManager.ResourceBarrier(context, debugDrawBuffer->GetResource(), VKW::RESOURCE_ACCESS_GENERIC_WRITE, VKW::STAGE_TRANSFER);
    context.CmdFillBuffer(debugDrawBuffer->GetResource(), 0, sizeof(DebugDrawBuffer), 0);
}
...
void DebugPrimitivesPass::Render(RenderGraph& graph, VKW::Context& context)
{
    ...
    {
        DRE_GPU_SCOPE(DebugPrimitivesPrepare);
        {
            DRE_GPU_SCOPE(DebugPrimitivesClearBuffers);
            dependencyManager.ResourceBarrier(context, sortedDrawBuffer->GetResource(), VKW::RESOURCE_ACCESS_GENERIC_WRITE, VKW::STAGE_TRANSFER);
            dependencyManager.ResourceBarrier(context, counters->GetResource(), VKW::RESOURCE_ACCESS_GENERIC_WRITE, VKW::STAGE_TRANSFER);
            dependencyManager.ResourceBarrier(context, indirectArgs->GetResource(), VKW::RESOURCE_ACCESS_GENERIC_WRITE, VKW::STAGE_TRANSFER);

            context.CmdFillBuffer(sortedDrawBuffer->GetResource(), 0, sizeof(DebugDrawBuffer), 0);
            context.CmdFillBuffer(counters->GetResource(), 0, sizeof(DebugDrawCounters), 0);
            context.CmdFillBuffer(indirectArgs->GetResource(), 0, sizeof(DrawIndirectCommand) * uint(DebugDrawCommandType::COUNT), 0);
        }

        DRE::U32 constexpr countGroupCount = DEBUG_DRAW_COMMAND_CAPACITY / 64;
        {
            DRE_GPU_SCOPE(DebugPrimitivesCountPrimitives);
        ...
        {
            DRE_GPU_SCOPE(DebugPrimitivesFillArgs);
...

    {
        ...
        context.CmdBeginRendering(attachmentsCount, renderTargets, nullptr, nullptr);
