    ...
    , swapchain_{ swapchain }
{
    ...
    for (std::uint8_t i = 0; i < swapchainImagesCount; ++i) {
        swapchainResources_.EmplaceBack(VKW::ImageResource{ swapchain->GetImage(i).image_, swapchain->GetFormat(), swapchain->GetWidth(), swapchain->GetHeight(), 1, VKW::MemoryRegion{}, swapchain->GetImageCreateInfo(), "SWAPCHAIN" });
